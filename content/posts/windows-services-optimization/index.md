...
[📥 下载 optimize_services.ps1](optimize_services.ps1)
...
### 2026-07-09 — 系统重置后重新优化

电脑重置后所有服务恢复默认。重新执行优化脚本，情况如下：

**成功禁用 26 个，失败 9 个**（失败的都是 Defender 相关服务）：

| 服务 | 状态 |
|------|------|
| 遥测服务、Windows Search、Superfetch | ✅ 已禁用 |
| 推送通知、错误报告、地图管理器、地理位置 | ✅ 已禁用 |
| Xbox 认证、Xbox 网络、Xbox 配件 | ✅ 已禁用 |
| Defender 防火墙、Defender 防病毒、安全中心 | ❌ 被篡改防护拦截 |

**原因**：Win11 24H2 默认开启「篡改防护」（Tamper Protection），阻止修改 Defender 服务状态。需要先关闭：
```
设置 → Windows 安全中心 → 病毒和威胁防护 → 管理设置 → 篡改防护 → 关
```
关闭后再运行脚本即可禁用 Defender 相关服务。

**备注**：火绒、Everything、Steam 等第三方服务重置后未安装，脚本自动跳过。

---

*注：本文是 2026-07-02 的实际折腾记录，2026-07-09 更新了系统重置后的重新优化体验。*
